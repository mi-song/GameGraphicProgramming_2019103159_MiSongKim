...
namespace library
{
	...
	HRESULT RenderTexture::Initialize(_In_ ID3D11Device* pDevice, _In_ ID3D11DeviceContext* pImmediateContext)
	{
		...
		hr = pDevice->CreateTexture2D(&textureDesc, NULL, m_texture2D.GetAddressOf());
		if (FAILED(hr))
			return hr;
		...
		hr = pDevice->CreateRenderTargetView(m_texture2D.Get(),
			&renderTargetViewDesc, m_renderTargetView.GetAddressOf());
		if (FAILED(hr))
			return hr;
		...
		hr = pDevice->CreateShaderResourceView(m_texture2D.Get(),
			&shaderResourceViewDesc, m_shaderResourceView.GetAddressOf());
		if (FAILED(hr))
			return hr;
		...
		hr = pDevice->CreateSamplerState(&sampDesc,
			m_samplerClamp.GetAddressOf());
		if (FAILED(hr))
			return hr;
	}
